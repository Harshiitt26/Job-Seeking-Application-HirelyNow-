...
const MyJobs = () => {
  ...
  useEffect(() => {
    const fetchJobs = async()=>{
      try {
        const {data} = await axios.get("https://job-seeking-application-hirelynow-backend.onrender.com/api/v1/job/getmyjobs", {withCredentials: true})
        setMyJobs(data.jobs)
      } catch (error) {
...
// for Enabling Editing Mode
  ...
  const handleUpdateJob = async(jobId)=>{
    ...
    await axios.put(`http://localhost:3000/api/v1/job/update/${jobId}`, updatedJob , {withCredentials: true})
    .then((res)=>{
  ...
  const handleJobDelete = async(jobId) => {
    await axios.delete(`http://localhost:3000/api/v1/job/delete/${jobId}`, {withCredentials: true})
    .then((res)=>{
      toast.success(res.data.message)
        ...
        <div className="container">
          <h3>Your Posted Jobs</h3>
          {
            myJobs && myJobs.length > 0 ? (
                    ...
                    <div className="card" key={element._id}>
                      <div className="content">
                        <div className="short_fields">
                          ...
                          <div>
                            <span>Expired:</span>
                            <select disabled={editingMode !== element._id ? true : false}
                            value={element.expired} onChange={(e)=> handleInputChange(element._id , "expired" , e.target.value)}>
                        ...
                        <div>
                            ...
                            <textarea rows={5} disabled={editingMode !== element._id ? true : false}
                            value={element.description} onChange={(e)=> handleInputChange(element._id , "description" , e.target.value)}/>
                          ...
                          <div>
                            <span>Location:</span>
                            <textarea rows={5} disabled={editingMode !== element._id ? true : false}
                            value={element.location} onChange={(e)=> handleInputChange(element._id , "location" , e.target.value)}/>
             ...
             : (<p>You've not posted any job or may have deleted all the jobs!</p>)
          }
        </div>
      </div>
    </>
